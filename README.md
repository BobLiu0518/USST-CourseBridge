...
1. 身份托管：代理服务器持有鉴权凭据，代为完成身份认证并自动维护 Session。
2. 接口代理：针对平台 API 进行动态转发，并处理复杂的 OAuth 签名逻辑。
3. 增强体验：配备轻量级前端界面，过滤繁琐的前端资源，加速课程加载。
...
3. 观看课程：进入后系统已自动登录，可直接进行学期切换、课程检索、点播回放，所有交互均通过代理服务器完成。
